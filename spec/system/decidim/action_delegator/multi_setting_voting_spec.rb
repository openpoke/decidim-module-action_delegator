...
describe "User votes in elections with multiple settings" do
  ...
  let!(:setting3) { create(:setting, organization:, title: { en: "Setting 3" }, active: true, authorization_method: :email) }
  ...
  let!(:participant_in_setting3) { create(:participant, setting: setting3, email: user.email) }

  let(:census_settings1) { { "authorization_handlers" => { "delegations_verifier" => { "options" => { "setting" => setting1.id.to_s } } } } }
  let(:census_settings2) { { "authorization_handlers" => { "delegations_verifier" => { "options" => { "setting" => setting2.id.to_s } } } } }
  let(:census_settings3) { { "authorization_handlers" => { "delegations_verifier" => { "options" => { "setting" => setting3.id.to_s } } } } }
  ...
  let!(:election2) { create(:election, :published, :ongoing, :with_questions, component:, title: { en: "Election 2" }, census_manifest: "internal_users", census_settings: census_settings2) }
  let!(:election3) { create(:election, :published, :ongoing, :with_questions, component:, title: { en: "Election 3" }, census_manifest: "internal_users", census_settings: census_settings3) }
  ...
  context "when user is participant in all settings" do
    ...
    it "can vote in all elections after single verification", :slow do
      ...
      visit_election(election3)
      click_on "Vote"
      expect(page).to have_content(translated(election3.questions.first.body))
    end
  end
  ...
  context "when user is not in participants" do
    ...
    it "cannot vote in any election", :slow do
      ...
      click_on "Vote"
      expect(page).to have_content("Verify your identity")
      expect(page).to have_no_content(translated(election3.questions.first.body))
    end
  end
end
